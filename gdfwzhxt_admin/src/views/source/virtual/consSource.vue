  ...
  <div class="consInfoSource">
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="60%">
      <el-form label-width="140px">
        <el-row>
          <el-col :span="12">
            <el-form-item label="用户编号*">
              <el-input v-model="consInfo.consNo" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="所属变压器*">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="用户名称*">
              <el-input v-model="consInfo.consName" clearable />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="联系电话">
              <el-input v-model="consInfo.tel" clearable />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="应急变压器">
              <el-input
                v-model="consInfo.emergencyTransformerName"
                clearable
                prefix-icon="search"
                @click="selectTransformer3"
                readonly
              />
            </el-form-item>
          </el-col>
    ...
    <el-table :data="list" style="width: 100%; margin-bottom: 20px;">
      <el-table-column label="操作" align="center" width="200" #default="scope">
        <el-button type="primary" size="small" @click="editConsInfo(scope.row)">
          修改
        </el-button>
        <el-button
          type="danger"
          size="small"
          @click="deleteConsInfoById(scope.row)"
        >
          删除
        </el-button>
      </el-table-column>
...
//根据点击的是查询条件中的变压器文本框还是添加或修改模态窗中的文本框来设值并做出相应的处理
var transformerTextBoxFlag
...
const selectTransformer = () => {
  ...
  transformerTextBoxFlag = 1
}

//点击添加或修改模态窗口中的所属变压器触发
const selectTransformer2 = () => {
  //重置查询条件
  queryTransformerDto.value = {}
  getTransformerTypeItem()
  transformerFetchData()
  dialogVisibleTransformerSelect.value = true
  transformerTextBoxFlag = 2
}

//点击添加或修改应急变压器文本框触发
const selectTransformer3 = () => {
  //重置查询条件
  queryTransformerDto.value = {}
  getTransformerTypeItem()
  transformerFetchData()
  dialogVisibleTransformerSelect.value = true
  transformerTextBoxFlag = 3
}

//变压器单元格点击事件
const transformerHandleCellClick = row => {
  if (transformerTextBoxFlag == 1) {
    consQueryDto.value.transformerCode = row.transformerCode
    consQueryDto.value.transformerName = row.transformerName
  } else if (transformerTextBoxFlag == 2) {
    consInfo.value.transformerCode = row.transformerCode
    consInfo.value.transformerName = row.transformerName
  } else if (transformerTextBoxFlag == 3) {
    consInfo.value.emergencyTransformerCode = row.transformerCode
    consInfo.value.emergencyTransformerName = row.transformerName
  }
...
const consInfo = ref({
  ...
  tel: '', //联系电话
  transformerCode: '', //所属变压器局号
  transformerName: '', //所属变压器名称
  connectionDate: '', //接电日期
  balance: '', //账户余额
  consStatus: '', //用户状态（正常/欠费）
  totalConsumption: '', //总用电量（kWh）度
  remark: '', //备注
  emergencyTransformerCode: '', //应急变压器局号
  emergencyTransformerName: '', //应急变压器名称
})
const dialogVisible = ref(false) //控制添加修改文本框开闭
const dialogTitle = ref('') //控制添加修改文本框标题

//点击添加按钮触发
const addShow = () => {
  consInfo.value = {}
  dialogTitle.value = '录入用户信息'
  dialogVisible.value = true
}

//点击修改按钮后触发
const editConsInfo = row => {
  consInfo.value = { ...row }
...
const saveOrUpdate = async () => {
  if (consInfo.value.consNo == undefined) {
    ElMessage.warning('【用户编号】不能为空')
    return
  }
  if (consInfo.value.transformerCode == undefined) {
    ElMessage.warning('【所属变压器】不能为空')
    return
  }
  if (consInfo.value.consName == undefined) {
    ElMessage.warning('【用户名称】不能为空')
    return
  }
  if (consInfo.value.consType == undefined) {
    ElMessage.warning('【用户类型】不能为空')
    return
  }
  if (consInfo.value.consStatus == undefined) {
    ElMessage.warning('【用户状态】不能为空')
    return
  }

  const { code, message } = await SaveConsInfo(consInfo.value)
  if (code === 200) {
    //关闭弹窗
...
//-----------------------------------------------------------------删除用户----------------------------------------------
const deleteConsInfoById = row => {
  ElMessageBox.confirm('此操作将永久删除该记录, 是否继续?', 'Warning', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning',
  }).then(async () => {
    const { code, message } = await DeleteConsInfoById(row.id)
    if (code === 200) {
      ElMessage.success(message)
      fetchData()
    } else {
