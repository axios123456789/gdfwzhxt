<template>
  <div class="workOrderAcceptCenterDT">
    <h2
...
workOrderQueryDto.value.workOrderStatus[0] = 3 //默认查询状态为待接单
...
//-----------------------------------------工单流程------------------------------------------
//点击接单
const acceptWorkOrder = row => {
  ElMessageBox.confirm(
    '此操作将会将该工单转派给你，此后该工单将完全由你负责，您确认要接受该工单吗?',
    'Warning',
    {
    ...
    const { code, message } = await AcceptWorkOrder(row)
    if (code === 200) {
      ElMessage.success(message + ', 您已成功接受该工单，请尽快完成！')
      fetchData()
    } else {
...
.workOrderAcceptCenterDT::before {
  content: '';
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  background-image: url('src/assets/bkm3.gif');
